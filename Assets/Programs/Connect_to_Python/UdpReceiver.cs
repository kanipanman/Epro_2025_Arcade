...
{
    public int listenPort = 5005; // Pythonと合わせる
    ...
    void Start()
    {
        try
        {
            ...
            receiveThread.Start();
            Debug.Log($"[UdpReceiver] Started listening on port {listenPort}");
        }
        catch (Exception e)
        {
    ...
    void Update()
    {
        while (queue.TryDequeue(out string json))
        {
            try
            {
                ...
                if (packet?.markers == null) continue;
                ...
                foreach (var m in packet.markers)
                {
                    ...
                    // --- PlayerAreaTracker に登録 ---
                    PlayerAreaTracker.SetPlayerArea(playerNum, area);
